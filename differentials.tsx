...
export function Differentials() {
  ...
  return (
    <section id="diferenciais" className="relative bg-background py-20 sm:py-24 lg:py-32 overflow-hidden">
      ...
      />
      <motion.div
        className="pointer-events-none absolute -top-32 right-0 h-80 w-80 rounded-full bg-solidy-blue/5"
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6">
        ...
        <motion.div
          ...
          className="mx-auto mt-10 sm:mt-12 max-w-3xl overflow-hidden rounded-2xl border border-border bg-card shadow-xl"
        >
          {/* Table header */}
          <div className="grid grid-cols-3 border-b border-border bg-solidy-blue/5 px-3 py-3 sm:px-6 sm:py-4">
            <p className="text-xs font-medium text-muted-foreground sm:text-sm">Recurso</p>
            <p className="text-center text-xs font-bold text-solidy-yellow sm:text-sm">Solidy</p>
            <p className="text-center text-xs font-medium text-muted-foreground sm:text-sm">Tradicional</p>
          </div>
          ...
          >
            {comparisonData.map((row, i) => (
              <motion.div
                ...
                className={`grid grid-cols-3 items-center px-3 py-3 sm:px-6 sm:py-4 transition-colors hover:bg-solidy-yellow/5 ${
                  i < comparisonData.length - 1 ? "border-b border-border" : ""
                }`}
              >
                <p className="text-xs text-foreground sm:text-sm pr-2">{row.feature}</p>
                <div className="flex justify-center">
                  <motion.div
                    ...
                    className="flex h-6 w-6 sm:h-7 sm:w-7 items-center justify-center rounded-full bg-solidy-green/15"
                  >
                ...
                <div className="flex justify-center">
                  <motion.div
                    ...
                    className={`flex h-6 w-6 sm:h-7 sm:w-7 items-center justify-center rounded-full ${
                      row.traditional ? "bg-solidy-green/15" : "bg-destructive/10"
                    }`}
                  >
                    {row.traditional ? (
                      <Check className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-solidy-green" />
          ...
          <div className="mt-6 sm:mt-8 flex flex-wrap items-center justify-center gap-3 sm:gap-4 lg:gap-6">
            {partners.map((partner, i) => (
              <motion.div
                ...
                className="rounded-xl bg-muted/60 px-4 py-2.5 sm:px-6 sm:py-3 text-xs sm:text-sm font-semibold text-muted-foreground transition-colors hover:bg-solidy-yellow/10 hover:text-solidy-blue cursor-default border border-transparent hover:border-solidy-yellow/20"
              >
                {partner}
              </motion.div>
            ))}
